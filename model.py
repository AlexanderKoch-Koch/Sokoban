...
class Model:
    ...
    def __init__(self, scope, observation_shape=(10, 10), num_actions=4, debug=True):
        ...
        with tf.variable_scope(scope):
            ...
            with tf.name_scope("value_head"):
                ...
                self.tf_critic_output = tf.squeeze(tf.add(tf.matmul(self.features_outputs, tf_critic_w3), tf_critic_b3), name="state_value_prediction")
                self.tf_critic_y = tf.placeholder(dtype=tf.float32, shape=(None,), name="true_state_value")
                self.tf_critic_loss = tf.reduce_mean(tf.square(tf.subtract(self.tf_critic_output, self.tf_critic_y), name="critic_loss"))
            ...
            self.gradients, self.grad_norms = tf.clip_by_global_norm(tf.gradients(self.loss, self.local_variables), 40.0)
    ...
    def predict(self, tf_session, observation):
        ...
        policy_output, value_output = tf_session.run([self.tf_actor_output, self.tf_critic_output], feed_dict={self.tf_input: observation})
    ...
    def train(self, tf_session, trajectory, step, summary_writer=None):
        ...
        _, summaries = tf_session.run([self.tf_apply_grads, self.tf_merged_summaries], feed_dict=feed_dict)
